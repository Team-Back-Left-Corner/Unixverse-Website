...
<div class="dashboard">
    <h1>Quiz Two Results: </h1>
            ...
            <?php
                ...
                if ($answers[0] == "A") { $totalCorrect++; }
                if ($answers[1] == "A") { $totalCorrect++; }
                if ($answers[2] == "A") { $totalCorrect++; }
                if ($answers[3] == "A") { $totalCorrect++; }
                if ($answers[4] == "A") { $totalCorrect++; }

                $totalPercentage = calculatePercentage($totalCorrect, $total);

                //Check the percentage
                if((int)$_SESSION['quiz_two_hs'] < $totalPercentage){
                    echo "New high score!<br>";
                    $_SESSION['quiz_two_hs'] = $totalPercentage;
                }

                $sql = "UPDATE `unixverse_users` SET `quiz_two_hs` = ? WHERE `id` = ?;";
